 ...
 *
 * This lives under `src/` (not `__tests__/`) so it is importable from benchmark
 * bundles, profiling fixtures, and test runtimes. It has no renderer imports,
 * so bundling it costs nothing unless a module actually imports it.
 */
...
export interface LongThreadFixture {
  ...
  messages: (FixtureMessage & { toolCalls: FixtureToolCall[] })[];
...
export const RENDERER_BENCHMARK_FIXTURE_VERSION = 'long-thread-v2' as const;

export interface RendererFeatureInventory {
  markdown: number;
  code: number;
  table: number;
  toolCall: number;
...
export interface RendererBenchmarkFixture extends LongThreadFixture {
  fixtureVersion: typeof RENDERER_BENCHMARK_FIXTURE_VERSION;
  featureInventory: RendererFeatureInventory;
  checksum: string;
}

export interface RendererBenchmarkFixturePair {
  fixtureVersion: typeof RENDERER_BENCHMARK_FIXTURE_VERSION;
  a: RendererBenchmarkFixture;
  b: RendererBenchmarkFixture;
}

export interface RendererBenchmarkState {
  fixtureKey: 'a' | 'b';
...
export function makeLongThread(options: LongThreadOptions = {}): LongThreadFixture {
  ...
  const messages: (FixtureMessage & { toolCalls: FixtureToolCall[] })[] = [];
...
export function benchmarkStateChecksum(
  ...
  return checksumText(
    JSON.stringify({
      ...
      fixtureChecksum: fixtures[state.fixtureKey].checksum,
      ...state,
    }),
  );
}

function addRequiredDiff(fixture: LongThreadFixture): LongThreadFixture {
  const messages = fixture.messages.map((message, index) => {
    if (index !== 1 || message.role !== 'assistant') return message;
    return {
      ...message,
      content: [
        message.content,
  ...
  return {
    ...fixture,
    messages,
    markdownCorpus: messages
      .filter((message) => message.role === 'assistant')
      .map((message) => message.content),
  };
}

function featureInventory(fixture: LongThreadFixture): RendererFeatureInventory {
  const corpus = fixture.markdownCorpus.join('\n');
  return {
    markdown: fixture.markdownCorpus.length,
    code: (corpus.match(/```(?:ts|tsx|bash|json)\n/g) ?? []).length,
    table: (corpus.match(/\| Area \| Current \| Candidate \|/g) ?? []).length,
    toolCall: fixture.counts.toolCalls,
    diff: (corpus.match(/```diff\n/g) ?? []).length,
  };
}

function rendererFixture(seed: number, threadId: string): RendererBenchmarkFixture {
  const fixture = addRequiredDiff(
    makeLongThread({ messageCount: 500, seed, threadId, toolCallRatio: 0.5 }),
  );
  const inventory = featureInventory(fixture);
  return {
    ...fixture,
    fixtureVersion: RENDERER_BENCHMARK_FIXTURE_VERSION,
    featureInventory: inventory,
    checksum: checksumText(JSON.stringify({ messages: fixture.messages, inventory })),
  };
}

export function makeRendererBenchmarkFixtures(): RendererBenchmarkFixturePair {
  return {
    fixtureVersion: RENDERER_BENCHMARK_FIXTURE_VERSION,
    a: rendererFixture(1, 'profile-a'),
    b: rendererFixture(2, 'profile-b'),
  };
}
