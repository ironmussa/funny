...
export function SchedulerView() {
  ...
  const refreshing = useSchedulerStore((s) => s.refreshing);
  const lastError = useSchedulerStore((s) => s.lastError);
  const loadRuns = useSchedulerStore((s) => s.loadRuns);
  ...
  return (
    <div className="flex h-full min-w-0 flex-1 flex-col" data-testid="scheduler-view">
      <div className="border-border flex items-center gap-3 border-b px-4 py-3">
        <div className="min-w-0 flex-1">
          <h2 className="flex items-center gap-2 text-sm font-medium">
            <Activity className="icon-sm text-muted-foreground" /> Scheduler queue
          </h2>
          <p className="text-muted-foreground mt-0.5 text-xs">
            Active runs claimed by the scheduler. Updates in real time.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <ScrollArea className="min-h-0 flex-1">
        {loading && runs.length === 0 ? (
          <LoadingState testId="scheduler-loading" label="Loading…" />
        ) : runs.length === 0 ? (
          <div className="text-muted-foreground px-4 py-12 text-center text-xs">
            No runs in flight. The scheduler picks up queued threads on the next tick.
          </div>
        ) : (
            ...
            {runs.map((run) => {
              ...
              return (
                <li
                  ...
                  data-testid={`scheduler-run-${run.threadId}`}
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        data-testid={`scheduler-run-open-${run.threadId}`}
                      >
