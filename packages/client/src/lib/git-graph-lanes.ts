 ...
 *
 * Given commits in child-before-parent order (the graph log uses
 * `git log --date-order` so commits sort by date like GitKraken — both
 * `--date-order` and `--topo-order` keep every parent below its children, which
 * is all this layout needs), assign each commit a horizontal "lane" and compute
 * the line segments to draw within each row's vertical band so the branch/merge
 * topology reads like GitKraken / VS Code's Git Graph.
 *
