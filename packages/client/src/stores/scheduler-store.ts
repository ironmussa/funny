...
import type { SchedulerRun } from '@/lib/api/scheduler';
import { createClientLogger } from '@/lib/client-logger';

const log = createClientLogger('scheduler-store');

export interface SchedulerState {
  runsByThread: Record<string, SchedulerRun>;
  loading: boolean;
  refreshing: boolean;
...
function patchRun(
  prev: SchedulerRun | undefined,
  threadId: string,
  patch: Partial<SchedulerRun>,
): SchedulerRun {
  const now = Date.now();
  if (!prev) {
    return {
...
export const useSchedulerStore = create<SchedulerState>((set, get) => ({
  ...
  loadRuns: async () => {
    ...
    const result = await api.listSchedulerRuns();
    result.match(
      (response) => {
        const byThread: Record<string, SchedulerRun> = {};
        for (const r of response.runs) byThread[r.threadId] = r;
        set({ runsByThread: byThread, loading: false });
      },
      (error) => {
        log.warn('Failed to load scheduler runs', { error: error.message });
  ...
  refresh: async () => {
    ...
    const result = await api.refreshScheduler();
    result.match(
      () => {
      ...
      (error) => {
        log.warn('Failed to refresh scheduler', { error: error.message });
