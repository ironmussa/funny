/**
 * OpenCodeACPProcess — adapter that wraps the opencode CLI (https://opencode.ai)
 * behind the IAgentProcess EventEmitter interface, communicating via the Agent
 * Client Protocol (ACP) over stdio.
 *
 * Spawns `opencode acp` as a subprocess and translates ACP session updates into
 * CLIMessage format so AgentMessageHandler works unchanged (same pattern as
 * CursorACPProcess and GeminiACPProcess).
 *
 * Authentication: the user runs `opencode auth login` once on the runner host;
 * opencode persists credentials to its own config dir. funny injects no
 * opencode credentials — same trust boundary as Cursor. Discovery surfaces an
 * `auth_required` state when the user hasn't logged in (see opencode-discover.ts).
 *
 * Model selection: opencode's catalog is discovered at runtime via
 * `opencode-discover.ts` (`session/new` advertises `models.availableModels`,
 * identical shape to cursor) and persisted as `provider:model` keys on the
 * client. opencode does NOT advertise the `setSessionModel` capability, but it
 * implements the raw `session/set_model` ACP method (spike-confirmed); we call
 * it via the SDK's `extMethod` escape hatch. The reasoning effort is encoded as
 * a model-id suffix (`opencode/gpt-5-nano/high`), which opencode parses itself —
 * funny passes the full wire-format id through `resolveModelId('opencode', …)`.
 *
 * Modes: opencode advertises exactly two session modes — `build` (default;
 * executes tools per configured permissions) and `plan` (disallows edits). We
 * map funny's `plan` permission mode → opencode `plan`; everything else → `build`,
 * set via `setSessionMode` after newSession.
 *
 * Permissions: mirrors `cursor-acp.ts` — checks funny's persisted permission
 ...
 * (funny's full-bypass) the handler short-circuits to allow without consulting
 * rules, matching the bypass semantics of other providers.
 *
...
const dlog = createDebugLogger('acp-opencode');
...
/**
 * opencode built-in tools surfaced via system:init. ACP doesn't expose a
 * listTools API, so this list is best-effort based on opencode's public docs.
...
/** Map funny's permissionMode → opencode session mode id (`build` | `plan`). */
function resolveSessionMode(permissionMode: string | undefined): 'build' | 'plan' {
  return permissionMode === 'plan' ? 'plan' : 'build';
}

export class OpenCodeACPProcess extends BaseAgentProcess {
  ...
  protected async runProcess(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        'ACP SDK not installed. Run: bun add @agentclientprotocol/sdk\n' +
          'Also ensure opencode is installed: curl -fsSL https://opencode.ai/install | bash\n' +
          'See https://opencode.ai/docs/acp for details.',
      );
    }

    const { ClientSideConnection, ndJsonStream } = SDK;

    const { command, args } = this.resolveOpenCodeAcpCommand();
    dlog.info('spawning opencode acp', { command, args, cwd: this.options.cwd });
      ...
      if (!this._exited && !this.isAborted) {
        if (err.code === 'ENOENT') {
          this.emit(
            'error',
            new Error(
              "'opencode' binary not found in PATH or failed to spawn.\n" +
                'Install via: curl -fsSL https://opencode.ai/install | bash\n' +
                'Or set OPENCODE_BIN to a custom location.\n' +
                'See https://opencode.ai/docs/acp for details.',
            ),
          );
        } else {
    ...
    child.on('exit', (code, signal) => {
      if (!this.isAborted && !this._exited) {
        dlog.warn('opencode child exited unexpectedly', { code, signal });
    ...
    try {
      ...
      // Select the requested model via the raw `session/set_model` ACP method
      // (opencode implements it but doesn't advertise the capability flag, so we
      // use the SDK's `extMethod` escape hatch). The sentinel `default` means
      // "use opencode's configured default" — skip the call.
      ...
      if (requestedModel && requestedModel !== 'default') {
        try {
          ...
          dlog.warn('session/set_model failed — falling back to opencode default', {
            modelId: requestedModel,
      ...
      // Switch session mode (opencode default is `build`). Only meaningful on a
      // fresh session — a resumed session keeps its previously approved mode.
      if (!isResume) {
        ...
        try {
          ...
          dlog.info('session/set_mode applied', { modeId: desiredMode });
        } catch (e) {
          dlog.warn('session/set_mode failed — staying on opencode default mode', {
            modeId: desiredMode,
            error: (e as Error)?.message,
          });
        }
  ...
  protected async runOnePrompt(prompt: string, images?: unknown[]): Promise<void> {
    if (!this.connection || !this.activeSessionId) {
      throw new Error('OpenCodeACPProcess: connection not initialized');
    }
  ...
  /**
   * Handle an ACP `session/request_permission` from opencode.
   *
   * Mirrors cursor-acp.ts so the existing UI (PermissionApprovalCard) and
   * persisted "always allow / always deny" rules light up unchanged:
   *
   * 1. In `autoEdit` mode, short-circuit to allow (funny's full-bypass
  ...
  private async handleRequestPermission(
    ...
    const denialText =
      `opencode requested permissions to use ${toolName} but the user hasn't been granted approval. ` +
  ...
  private translateUpdate(update: ACPSessionUpdate): void {
    ...
    switch (update.sessionUpdate) {
      ...
      case 'tool_call': {
        ...
        // call that still lacks it is dropped rather than emitted as an empty
        // card — the server freezes a tool's input at first emission.
      ...
      case 'tool_call_update': {
        ...
        // opencode can fire a completed tool_call_update without an emitted
  ...
  private resolveOpenCodeAcpCommand(): { command: string; args: string[] } {
 ...
 * auto-approve every `session/request_permission` so opencode never pauses.
 */
