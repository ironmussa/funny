/**
 * Unit tests for HttpSchedulerClient.
 */

import { describe, expect, test } from 'bun:test';

import { HttpClientError, HttpSchedulerClient } from '../../adapters/http-client.js';
...
describe('HttpSchedulerClient', () => {
  test('GET request includes auth header and parses JSON', async () => {
    ...
    expect(calls[0].url).toBe('http://server:3001/api/scheduler/system/runs');
    expect(calls[0].headers['x-scheduler-auth']).toBe('secret-xyz');
  });
