 ...
 *
 * Shared `fetch` wrapper for the scheduler brain → funny server
 * (`/api/scheduler/system/*`). Adds the `X-Scheduler-Auth` header
...
export interface HttpClientOptions {
  ...
  baseUrl: string;
  /** Shared secret matching `SCHEDULER_AUTH_SECRET` on the server. */
  authSecret: string;
  /**
...
export class HttpSchedulerClient {
  ...
  /**
   * Issue a request. `path` is relative to `/api/scheduler/system`,
   * e.g. `/runs` becomes `<baseUrl>/api/scheduler/system/runs`.
   */
  async request<T = unknown>(
    ...
    const url = new URL(this.baseUrl + '/api/scheduler/system' + path);
    if (opts.query) {
    ...
    const headers: Record<string, string> = {
      'X-Scheduler-Auth': this.authSecret,
    ...
    const parsed = parseStoredJson(z.unknown(), text, 'scheduler HTTP response');
    if (!parsed.ok) {
