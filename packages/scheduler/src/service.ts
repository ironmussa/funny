 ...
 * @domain depends: SchedulerRunRepository, ThreadQueryAdapter, Dispatcher
 *
 * Scheduler brain. Owns the poll/reconcile loops, builds the inputs
 * for `planDispatch` (pure logic in @funny/core/scheduler), and applies
 ...
 * server; with HTTP adapters this runs inside the standalone scheduler
 * binary against `/api/scheduler/system/*`.
 */
...
import {
  ...
  type RetryEntry,
  type RunRef,
} from '@funny/core/scheduler';
import type { Thread } from '@funny/shared';
import type { SchedulerRunRepository } from '@funny/shared/repositories';
...
export interface SchedulerServiceDeps {
  runRepo: SchedulerRunRepository;
...
const NS = 'scheduler-service';

export class SchedulerService {
  private readonly runRepo: SchedulerRunRepository;
  ...
  start(): void {
    if (!this.config.enabled) {
      this.log.info('Scheduler disabled by config — not starting', { namespace: NS });
      return;
    }
  ...
  async stop(): Promise<void> {
    ...
    this.log.info('Scheduler stopped', { namespace: NS });
  }
  ...
  private async safeReconcile(): Promise<void> {
    try {
      ...
      this.log.error('Scheduler reconcile failed', {
        namespace: NS,
        error: err instanceof Error ? err.message : String(err),
      });
    }
