/**
 * Phase 3 integration tests for SchedulerService.
 *
...
import { createThreadQuery } from '../../services/scheduler-thread-query.js';
import { createTestDb, seedProject, seedThread } from '../helpers/test-db.js';

// ── Helpers ───────────────────────────────────────────────────

const silentLog: SchedulerLogger = {
  info: () => {},
  warn: () => {},
  error: () => {},
};
...
function makeEmitter(): SchedulerEmitter & { events: RecordedEvent[] } {
...
let runRepo: ReturnType<typeof createSchedulerRunRepository>;
...
beforeEach(() => {
  testDb = createTestDb();
  runRepo = createSchedulerRunRepository({
    db: testDb.db as any,
...
function makeService(overrides: Partial<typeof defaultSchedulerConfig> = {}, now?: () => number) {
  return new SchedulerService({
    ...
    dispatcher,
    emitter,
    config: { ...defaultSchedulerConfig, enabled: true, ...overrides },
    log: silentLog,
    now,
  });
}
...
describe('SchedulerService.refresh', () => {
  test('dispatches an eligible thread and writes scheduler_runs', async () => {
    ...
    expect(types).toContain('scheduler:tick');
  });
  ...
  test('start() is a no-op when feature flag is disabled', () => {
    const svc = new SchedulerService({
      ...
      dispatcher,
      emitter,
      config: { ...defaultSchedulerConfig, enabled: false },
      log: silentLog,
    });
    svc.start();
...
describe('SchedulerService — completion handling', () => {
...
describe('SchedulerService — stall detection', () => {
  test('aborts a stalled in-flight handle and emits thread:stalled', async () => {
    ...
    // Skip past the stall window. The scheduler_runs row was
