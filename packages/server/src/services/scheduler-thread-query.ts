 ...
 *
 * SQL adapter that backs the scheduler's eligibility queries.
...
import type { ThreadQueryAdapter } from '@funny/thread-scheduler';
...
export function createThreadQuery(deps: ThreadQueryDeps): ThreadQueryAdapter {
  ...
  return {
    async listEligibleCandidates(): Promise<Thread[]> {
      ...
      // scheduler_runs (means we've already claimed it). Doing
      ...
      const claimed = await dbAll<{ threadId: string }>(
        db.select({ threadId: schema.schedulerRuns.threadId }).from(schema.schedulerRuns),
      );
      const claimedIds = claimed.map((c) => c.threadId);

      const filters = [
        eq(schema.threads.schedulerManaged, 1),
