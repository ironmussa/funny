 ...
 * scanner (`watchers.next_wake_at`) and the orchestrator run state
 * (`orchestrator_runs.*_at_ms`). These columns must be bigint on Postgres.
...
import { orchestratorRuns as pgOrchestratorRuns, watchers as pgWatchers } from '../db/schema.pg.js';
import {
  orchestratorRuns as sqliteOrchestratorRuns,
...
describe('epoch-ms columns are bigint on Postgres', () => {
  ...
  test('orchestrator_runs *_at_ms columns are bigint', () => {
    const cols = getTableColumns(pgOrchestratorRuns);
  ...
  test('SQLite keeps these columns as integer (8-byte, no overflow)', () => {
    ...
    const o = getTableColumns(sqliteOrchestratorRuns);
