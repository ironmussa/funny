...
import { createSchedulerRunRepository } from '../../repositories/scheduler-run-repository.js';
import { createTestDb, seedProject, seedThread } from '../helpers/test-db.js';

let deps: ReturnType<typeof createTestDb>;
let repo: ReturnType<typeof createSchedulerRunRepository>;

beforeEach(() => {
  deps = createTestDb();
  repo = createSchedulerRunRepository(deps);
  seedProject(deps.db);
