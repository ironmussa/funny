 ...
 *
 * DB-agnostic repository for the scheduler's run state.
 * Manages two tables:
 *   - `scheduler_runs`     — one row per actively claimed thread
...
export function createSchedulerRunRepository(deps: SchedulerRunRepositoryDeps) {
  const { db, schema, dbAll, dbGet, dbRun } = deps;
  const table = schema.schedulerRuns;
   ...
   */
  async function claim(args: ClaimArgs): Promise<SchedulerRunRow> {
  ...
  async function getRun(threadId: string): Promise<SchedulerRunRow | undefined> {
    return dbGet<SchedulerRunRow>(db.select().from(table).where(eq(table.threadId, threadId)));
  }

  async function listActiveRuns(): Promise<SchedulerRunRow[]> {
    return dbAll<SchedulerRunRow>(db.select().from(table));
  }

  async function listActiveRunsByUser(userId: string): Promise<SchedulerRunRow[]> {
    return dbAll<SchedulerRunRow>(db.select().from(table).where(eq(table.userId, userId)));
  }
  ...
  async function listDueRetries(now: number): Promise<SchedulerRunRow[]> {
    return dbAll<SchedulerRunRow>(
      db
        .select()
        .from(table)
...
export type SchedulerRunRepository = ReturnType<typeof createSchedulerRunRepository>;
