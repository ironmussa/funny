/**
 * YAML workflow parser + schema tests.
 *
 * Validates the boundary contract: well-formed YAML with valid schema
 * yields a typed `ParsedWorkflow`; malformed inputs yield structured
...
describe('parseWorkflowYaml', () => {
  test('accepts a minimal valid workflow', () => {
    const result = parseWorkflowYaml(validMinimal);
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.workflow.name).toBe('minimal');
      expect(result.workflow.nodes).toHaveLength(1);
      expect(result.workflow.nodes[0].notify?.message).toBe('hello');
    }
  });

  test('rejects malformed YAML with a diagnostic message', () => {
    const result = parseWorkflowYaml('name: foo\n  bad indent: [\n');
  ...
  test('rejects workflows with zero nodes', () => {
    const result = parseWorkflowYaml('name: empty\nnodes: []');
    expect(result.ok).toBe(false);
    if (!result.ok) {
...
`;
  ...
  test('rejects invalid workflow names (uppercase, special chars)', () => {
    const yaml = `
name: BadName
nodes:
  - id: a
...
`;
  ...
  test('accepts a full workflow with all action types', () => {
    const yaml = `
name: full
...
`;
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.workflow.nodes[0].set_status?.value).toBe('running');
      expect(result.workflow.nodes[0].set_status?.reason).toBe('kicked off');
      expect(result.workflow.nodes[1].set_stage?.value).toBe('in_progress');
    }
  });
...
`;
  ...
  test('formatParseError renders the issues as multi-line text', () => {
    const result = parseWorkflowYaml('name: BadName\nnodes: []');
    expect(result.ok).toBe(false);
    if (!result.ok) {
