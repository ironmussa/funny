...
| `check-runner-transport.ts`           | gRPC remains the only runner data plane: no runner Socket.IO namespace, removed transport configuration, runtime Socket.IO dependency, or direct runner HTTP routing may return.                                                                                                                                   |
...
| `check-json-boundary-validation.ts`   | No NEW direct `JSON.parse` calls in server/runtime/shared/core/scheduler/memory source without explicit baseline refresh. New persisted or external JSON consumers should use `parseStoredJson` or `parseExternalPayload`.                                                                                            |
